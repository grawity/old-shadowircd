 ...
 *  USA
 *
 *  $Id: dh.c,v 1.1 2003/11/04 07:11:07 nenolod Exp $
 *
 */
...
make_entropy ()
{
  ...
  printf
    ("§ No random state found, trying to generate entropy from /dev/random...\n");
  printf ("§ This may take a moment.\n");
  printf
    ("§ To speed up this process, do something else on the system for a bit.\n");
  ...
  if (!fp)
    {
      ...
      printf
	("§ You can place a file containing random data called .ircd.entropy\n");
      printf
	("§ in the directory with your ircd.conf. This file must be at least %d bytes\n",
	 RAND_BYTES);
