...
tokenize(char *message, char **parv)
{
     ...
     * somewhere so we can set it to the last param in parv also make sure 
     * there's a space before it... if not then we're screwed 
     */
    pos = message;
    while (1) {
	if ((pos = strchr(pos, ':'))) {
	...
	if (count == 19) {
	    /*
	     * we've reached one less than our max limit to handle the
	     * parameter we alreayd ripped off 
	     */
	...
	if (*next == ' ') {
	    *next = '\0';
		...
		next++;
	    /*
	     * if it's the end of the string, it's simply an extra space
	     * before the :parameter. break. 
	     */
	    if (*next == '\0')
		break;
	    parv[count] = next;
...
irc_parse(char *line)
{
	 ...
	 * those here... they'll core us later on if we don't 
	 */
	if (*line == '\n')
	    return;
	if (*line == '\000')
	...
	if ((pos = strchr(line, ' '))) {
	    *pos = '\0';
	    pos++;
	    /*
	     * if it starts with a : we have a prefix/origin pull the
	     * origin off into `origin', and have pos for the command,
	...
	/*
	 * okay, the nasty part is over, now we need to make a parv out of 
	 * what's left 
	 */
	...
	/*
	 * now we should have origin (or NULL), command, and a parv with
	 * it's accompanying parc... let's make ABSOLUTELY sure 
	 */
	if (!command) {
...
static void
m_pass(char *origin, uint8_t parc, char *parv[])
{
...
static void
m_eob(char *origin, uint8_t parc, char *parv[])
{
    /*
