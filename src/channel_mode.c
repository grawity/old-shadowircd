 ...
 *  USA
 *
 *  $Id: channel_mode.c,v 1.1.1.1 2004/09/07 04:50:33 nenolod Exp $
 */
...
static void chm_owner (struct Client *, struct Client *, struct Channel *,
		       int, int *, char **, int *, int, int, char, void *,
		       const char *chname);

static void chm_owneronly (struct Client *, struct Client *, struct Channel *,
			   int, int *, char **, int *, int, int, char, void *,
			   const char *chname);
...
static void chm_quiet (struct Client *, struct Client *, struct Channel *,
		       int, int *, char **, int *, int, int, char, void *,
		       const char *chname);

static void chm_restrict (struct Client *, struct Client *, struct Channel *,
			  int, int *, char **, int *, int, int, char, void *,
			  const char *chname);
  ...
  switch (type)
    ...
    default:
      sendto_realops_flags (UMODE_ALL, L_ALL,
			    "add_id() called with unknown ban type %d!",
			    type);
      return (0);
    }
...
del_id (struct Channel *chptr, const char *banid, int type)
{
  ...
  switch (type)
    ...
    default:
      sendto_realops_flags (UMODE_ALL, L_ALL,
			    "del_id() called with unknown ban type %d!",
			    type);
      return (0);
    }
...
pretty_mask (char *mask)
{
  ...
  mask_pos +=
    ircsprintf (mask_buf + mask_pos, "%s!%s@%s", nick, user, host) + 1;
...
static void
chm_owneronly (struct Client *client_p, struct Client *source_p,
...
static void
chm_operonly (struct Client *client_p, struct Client *source_p,
	      struct Channel *chptr, int parc, int *parn, char **parv,
	      int *errors, int alev, int dir, char c, void *d,
	      const char *chname)
{
...
/* XXX grrrrrrr */
	...
	{
	  /* mask isn't a valid ban, check raw_mask */
	  if ((del_id (chptr, raw_mask, CHFL_QUIET) == 0)
	      && MyClient (source_p))
	    {
	      /* nope */
	      return;
...
/* XXX grrrrrrr */
	...
	{
	  /* mask isn't a valid ban, check raw_mask */
	  if ((del_id (chptr, raw_mask, CHFL_RESTRICT) == 0)
	      && MyClient (source_p))
	    {
	      /* nope */
	      return;
	   ...
	   const char *chname)
{
  ...
  if (IsRestricted (source_p) && (dir == MODE_ADD))
	...
	sendto_one (source_p,
		    ":%s NOTICE %s :*** Notice -- You are restricted and cannot "
		    "chanowner others", me.name, source_p->name);
  ...
  /* no redundant mode changes */
  if (dir == MODE_ADD && has_member_flags (member, CHFL_CHANOWNER))
    return;
  if (dir == MODE_DEL && !has_member_flags (member, CHFL_CHANOWNER))
    return;

  mode_changes[mode_count].letter = 'u';
  mode_changes[mode_count].dir = dir;
  mode_changes[mode_count].caps = 0;
  mode_changes[mode_count].nocaps = 0;
  mode_changes[mode_count].mems = ALL_MEMBERS;
  ...
  if (dir == MODE_ADD)
    {
      AddMemberFlag (member, CHFL_CHANOWNER);
      DelMemberFlag (member, CHFL_DEOPPED);
    }
  else
    DelMemberFlag (member, CHFL_CHANOWNER);
}
...
static struct ChannelMode ModeTable[255] =
{
  ...
  {chm_simple, (void *) MODE_USEFILTER},          /* G */
  {chm_nosuch, NULL},                             /* H */
  {chm_invex, NULL},                              /* I */
  {chm_nosuch, NULL},                             /* J */
...
get_channel_access (struct Client *source_p, struct Membership *member)
{
  ...
  if (has_member_flags (member, CHFL_CHANOWNER))
    return (CHACCESS_CHANOWNER);
		       ...
		       struct Channel *chptr, int cap, int nocap)
{
  ...
  if ((cap & CAP_TS6) && source_p->user && (source_p->id[0] != '\0'))
		   ...
		   struct Channel *chptr, char *chname)
{
  ...
  for (i = 0; i < mode_count; i++)
	  ...
	  ((arglen + pbl + BAN_FUDGE) >= MODEBUFLEN))
	{
	  ...
	  if (nc != 0)
	    sendto_channel_local (ALL_MEMBERS, chptr, "%s %s", modebuf,
				  parabuf);
...
set_channel_mode (struct Client *client_p, struct Client *source_p,
		  struct Channel *chptr, struct Membership *member, int parc,
		  char *parv[], char *chname)
{
  int dir = MODE_ADD;
  int parn = 1;
  int alevel, errors = 0;
