...
m_nick (aClient * client_p, aClient * source_p, int parc, char *parv[])
{
   ...
   */
  if ((IsServer (source_p) || (parc > 4)) &&
      ((IsNICKIP (source_p) && (parc < 11))
       || (!IsNICKIP (source_p) && (parc < 10))))
    {
      /*
  ...
  if (((!IsNICKIP (source_p) && (parc == 10))
       || (IsNICKIP (source_p) && (parc == 11)))
#ifdef INET6
      && ((!strchr (parv[6], '.')) && (!strchr (parv[6], ':'))))
#else
      && (!strchr (parv[6], '.')))
#endif
  {
    ...
    sendto_one (client_p, ":%s KILL %s :Bad NICK with invalid hostname",
		me.name, parv[1]);
    return 0;
  }
  ...
  if (do_nick_name (nick) == 0
	...
	{
	  ircstp->is_kill++;
	  sendto_snomask (SNOMASK_DEBUG, "Bad Nick: %s From: %s %s",
			      parv[1], parv[0],
			      get_client_name (client_p, FALSE));
	  sendto_one (client_p, ":%s KILL %s :Bad Nick", me.name, parv[1]);
	  if (source_p != client_p)
	    {			/* bad nick change */
	      sendto_serv_butone (client_p, ":%s KILL %s :Bad Nick", me.name,
				  parv[0]);
  ...
  do
      ...
      if (target_p)
	{
	   ...
	   */
	  sendto_snomask (SNOMASK_DEBUG, "Nick collision on %s",
			      source_p->name);
	  ircstp->is_kill++;
	  sendto_one (client_p, ":%s KILL %s :Nick Collision", me.name,
		      source_p->name);
	  source_p->flags |= FLAGS_KILLED;
      ...
      if (IsUnknown (target_p))
	{
	  if (MyConnect (target_p))
	    ...
	    {
	      sendto_snomask (SNOMASK_KILLS, "Nick Collision on %s", parv[1]);
      ...
      if (IsServer (source_p))
	{
	  ...
	  if (!newts || !target_p->tsinfo || (newts == target_p->tsinfo))
		...
		{
		  sendto_snomask (SNOMASK_KILLS, "Nick collision on %s",
				      parv[1]);
		  ircstp->is_kill++;
		  sendto_one (target_p, err_str (ERR_NICKCOLLISION),
			      me.name, target_p->name, target_p->name);
		  sendto_serv_butone (source_p, ":%s KILL %s :Nick Collision",
				      me.name, target_p->name);
		  target_p->flags |= FLAGS_KILLED;
		  (void) exit_client (client_p, target_p, &me,
				      "Nick collision");
		  break;
		}
	    }
	  ...
	  || !source_p->user)
	{
	  sendto_snomask (SNOMASK_KILLS, "Nick change collision: %s",
			      parv[1]);
	  ircstp->is_kill++;
	  sendto_one (target_p, err_str (ERR_NICKCOLLISION),
		      me.name, target_p->name, target_p->name);
      ...
      else
	{
	  sameuser =
	    ((irccmp (target_p->user->username, source_p->user->username) ==
	      0)
	     && (irccmp (target_p->user->host, source_p->user->host) == 0));
	  if ((sameuser && newts < target_p->tsinfo)
	      || (!sameuser && newts > target_p->tsinfo))
	    {
	      if (sameuser)
		sendto_snomask (SNOMASK_KILLS,
	      ...
	      ircstp->is_kill++;
	      sendto_serv_butone (client_p, ":%s KILL %s :Nick Collision",
  ...
  if (IsServer (source_p))
      ...
      if (!uplink)
	{
	  ...
	  sendto_realops ("Remote nick %s on UNKNOWN server %s", nick,
			  parv[7]);
	  return 0;
	}
      ...
      if (parc >= 10)
	{
	  ...
	  if (parc == 11 && (!IsNICKIP (client_p)))
	    sendto_snomask (SNOMASK_DEBUG,
				"Extra Param without NICKIP from %s",
				client_p->name);
	  ...
	  if (parc == 10)
	    {
	      return do_user (nick, client_p, source_p, parv[5], parv[6],
			      parv[7], strtoul (parv[8], NULL, 0), 0,
			      parv[9], NULL);
	    }
	  else if (parc == 11)
	    {
	      return do_user (nick, client_p, source_p, parv[5], parv[6],
			      ...
			      strtoul (parv[9], NULL, 0), parv[10], NULL);
	    }
	}
    }
	  ...
	  if ((aconf = find_conf_name (nick, CONF_QUARANTINED_NICK)))
	    {
	      ...
	      if (!MyConnect (source_p))
		sendto_snomask (SNOMASK_REJECTS, "Q:lined nick %s from %s on %s",
				    nick, (*source_p->name != 0
					   && !IsServer (source_p)) ?
				    source_p->name : "<unregistered>",
				    (source_p->user ==
				     NULL) ? ((IsServer (source_p)) ? parv[6]
					      : me.name) : source_p->user->
				    server);
#endif
      ...
      if (MyConnect (source_p))
	{
	  if (IsRegisteredUser (source_p))
		...
		{
		  if (can_send (source_p, lp->value.channel_p))
		    {
		      sendto_one (source_p, err_str (ERR_BANNICKCHANGE),
				  me.name, source_p->name,
				  lp->value.channel_p->chname);
		      return 0;
		    }
		  if (nick_is_banned (lp->value.channel_p, nick, source_p) !=
		      NULL)
		    {
		...
		{
#endif
		  sendto_common_channels (source_p, ":%s NICK :%s", parv[0],
					  nick);
		  if (source_p->user)
		    {
		...
		{
		  sendto_one (source_p,
			      ":%s NOTICE %s :*** Notice -- Too many nick changes."
			      "Wait %d seconds before trying again.",
  ...
  if (source_p->name[0])
    {
      (void) del_from_client_hash_table (source_p->name, source_p);
...
m_client (aClient * client_p, aClient * source_p, int parc, char *parv[])
{
  ...
  if (parc < 13)
	...
	(SNOMASK_DEBUG,
	 ...
	 (parc >= 10) ? parv[9] : "-", parv[0]);
      ...
      sendto_one (client_p,
		  ":%s KILL %s :Bad CLIENT with wrong number of parameters",
		  me.name, parv[1]);
      return 0;
    }
  ...
  if ((!strchr (parv[7], '.')) && (!strchr (parv[7], ':')))
#endif
    {
      ...
      sendto_one (client_p, ":%s KILL %s :Bad CLIENT with invalid hostname",
		  me.name, parv[1]);
      return 0;
    }
  newts = atol (parv[3]);
  ...
  if (do_nick_name (nick) == 0 || strcmp (nick, parv[1]))
    {
      sendto_one (source_p, err_str (ERR_ERRONEUSNICKNAME),
		  me.name, parv[0], parv[1], "Erroneous Nickname", "N/A");
      ircstp->is_kill++;
      sendto_snomask (SNOMASK_DEBUG, "Bad Nick: %s in CLIENT From: %s %s",
  ...
  do
      ...
      if (IsUnknown (target_p))
	{
	  if (MyConnect (target_p))
	    ...
	    {
	      sendto_snomask (SNOMASK_DEBUG, "Nick Collision on %s", parv[1]);
	      sendto_serv_butone (NULL, ":%s KILL %s :Nick Collission",
				  me.name, target_p->name);
	      target_p->flags |= FLAGS_KILLED;
	      /* Having no USER struct should be ok... */
  ...
  if (IsServer (source_p))
      ...
      while (*mptr)
	{
      ...
      return do_user (nick, client_p, source_p, parv[6], parv[7], parv[9],
		      strtoul (parv[10], NULL, 0), strtoul (parv[11], NULL,
							    0), parv[12],
		      parv[8]);
