 ...
 *  hostmasking.c: IP Masking routines.
 *
 *  Obtained from Greg (Siracide) <sharpen@wanadoo.fr>
 *  Modified by J. Johnston
 *  Modified even further by W. Pitcock (nenolod) <nenolod@gmail.com>
 *
 ...
 *  USA
 *
 *  $Id: hostmasking.c,v 1.1 2004/07/29 15:27:30 nenolod Exp $
 */
...
make_virthost(char *curr, char *host, char *new)
{
	...
	if(parc2 == 4) {
		...
		if(strchr("0123456789", parv2[3][len - 1])) {
			...
			ircsprintf(mask, "%lx.%lx.%lx.%lx.IP", hash[0], hash[1], hash[2], hash[3]);
		} else {
		...
		if(parc2 >= 4) {	/* There are isp's like *.isp.co.uk etc out there */
			ircsprintf(mask, "%lx.%lx.%s.%s.%s",
			    hash[0], hash[1], parv2[parc2 - 3], parv2[parc2 - 2], parv2[parc2 - 1]);
		} else {
			ircsprintf(mask, "%lx.%lx.%s.%s",
			    hash[0], hash[1], parv2[parc2 - 2], parv2[parc2 - 1]);
		}
