 ...
 *  USA
 *
 *  $Id: hook.c,v 1.1 2004/07/29 15:27:20 nenolod Exp $
 */
...
int h_doing_whois_local_id;
int h_doing_whois_global_id;
int h_doing_admin_id;
int h_doing_info_id;
int h_doing_links_id;
int h_doing_motd_id;
int h_doing_stats_id;
int h_doing_stats_p_id;
int h_doing_trace_id;
...
init_hooks(void)
{
	...
	hook_add_event("doing_whois_local", &h_doing_whois_local_id);
	hook_add_event("doing_whois_global", &h_doing_whois_global_id);
	hook_add_event("doing_admin", &h_doing_admin_id);
	hook_add_event("doing_info", &h_doing_info_id);
	hook_add_event("doing_links", &h_doing_links_id);
	hook_add_event("doing_motd", &h_doing_motd_id);
	hook_add_event("doing_stats", &h_doing_stats_id);
	hook_add_event("doing_stats_p", &h_doing_stats_p_id);
	hook_add_event("doing_trace", &h_doing_trace_id);
}
...
hook_add_event(const char *name, int *id)
{
	...
	if((newhook = find_hook_byname(name)))
	{
		*id = newhook->id;
		return 0;
	}
