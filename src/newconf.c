...
static dlink_list yy_hubleaf_list;
...
static struct mode_table umode_table[] = {
	...
	{"hidden_oper", UMODE_HIDDENOPER},
	{NULL}
};

static struct mode_table flag_table[] = {
	{"local_kill", OPER_LOCAL_KILL},
	{"global_kill", OPER_LOCAL_KILL|OPER_GLOBAL_KILL},
	{"remote", OPER_REMOTE},
	{"kline", OPER_K},
...
static struct mode_table shared_table[] =
{
	...
	{ "xline",	OPER_XLINE	},
	{ "resv",	OPER_RESV	},
	{ "all",	OPER_K|OPER_UNKLINE|OPER_XLINE|OPER_RESV },
	{NULL}
};
...
conf_set_serverinfo_rsa_private_key_file(void *data)
{
	...
	if(file == NULL)
	{
		conf_report_error("Ignoring config file entry rsa_private_key -- file open failed"
				  " (%s)", (char *) data);
		return;
	}
...
conf_end_oper(struct TopConf *tc)
{
...
#ifdef HAVE_LIBCRYPTO
	if(EmptyString(yy_aconf->passwd) && EmptyString(yy_aconf->rsa_public_key_file))
#else
	if(EmptyString(yy_aconf->passwd))
#endif
	{
		conf_report_error("Ignoring operator block -- missing password");
		return 0;
	}
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, yy_aconf_list.head)
	{
		...
		if(!EmptyString(yy_aconf->className))
			DupString(yy_tmp->className, yy_aconf->className);

		yy_tmp->status = yy_aconf->status;
		yy_tmp->flags = yy_aconf->flags;
		yy_tmp->port = yy_aconf->port;

#ifdef HAVE_LIBCRYPTO
		if(!EmptyString(yy_tmp->rsa_public_key_file))
		{
			BIO *file;

			if((file = BIO_new_file(yy_aconf->rsa_public_key_file, "r")) == NULL)
			{
				conf_report_error("Ignoring operator block -- unable to open key");
				return 0;
			}
			...
			if(!yy_tmp->rsa_public_key)
			{
				conf_report_error("Ignoring operator block -- invalid key");
				return 0;
			}
...
conf_set_oper_flags(void *data)
{
	...
	set_modes_from_table(&yy_aconf->port, "flag", flag_table, args);
}
...
conf_set_oper_name(void *data)
{
	...
	if((strlen((const char *) data)) > OPERNICKLEN)
		((char *) data)[OPERNICKLEN] = '\0';

	DupString(yy_aconf->name, (char *) data);
}
...
conf_set_oper_user(void *data)
{
	...
	if(EmptyString(yy_tmp->user) || EmptyString(yy_tmp->host))
	{
		conf_report_error("Ignoring operator::user -- missing username/host");
		free_conf(yy_tmp);
		return;
	}
...
conf_set_oper_password(void *data)
{
	...
	DupString(yy_aconf->passwd, (char *) data);
}
...
conf_set_oper_class(void *data)
{
	...
	DupString(yy_aconf->className, (char *) data);
}
...
conf_set_oper_global_kill(void *data)
{
	...
	if(yesno)
		yy_aconf->port |= (OPER_GLOBAL_KILL|OPER_LOCAL_KILL);
	else
		yy_aconf->port &= ~(OPER_GLOBAL_KILL|OPER_LOCAL_KILL);
}

static void
conf_set_oper_local_kill(void *data)
{
	int yesno = *(int *) data;

	if(yesno)
		yy_aconf->port |= OPER_LOCAL_KILL;
	else
		yy_aconf->port &= ~ OPER_LOCAL_KILL;
}
...
conf_set_oper_hidden_admin(void *data)
{
	...
	if(yesno)
		yy_aconf->port |= OPER_HIDDENADMIN;
	else
		yy_aconf->port &= ~OPER_HIDDENADMIN;
}
...
conf_set_oper_spy(void *data)
{
	...
	if(yesno)
		yy_aconf->port |= OPER_SPY;
	else
		yy_aconf->port &= ~OPER_SPY;
}

static void
conf_set_oper_hidden_oper(void *data)
{
	int yesno = *(int *) data;

	if(yesno)
		yy_aconf->port |= OPER_HIDDENOPER;
	else
		yy_aconf->port &= ~OPER_HIDDENOPER;
}
...
conf_end_auth(struct TopConf *tc)
{
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, yy_aconf_list.head)
	{
		...
		/* if this doesnt exist, conf_add_class_to_conf() deals 
		 * with it fine..
		 */
		if(!EmptyString(yy_aconf->className))
			DupString(yy_tmp->className, yy_aconf->className);
...
conf_set_auth_user(void *data)
{
	...
	/* first doesnt allocate a new conf.. */
	if(!EmptyString(yy_aconf->host))
		yy_tmp = make_conf();
	else
		yy_tmp = yy_aconf;
...
conf_set_auth_flood_exempt(void *data)
{
	...
	if(yesno)
		yy_aconf->flags |= CONF_FLAGS_EXEMPTFLOOD;
	else
		yy_aconf->flags &= ~CONF_FLAGS_EXEMPTFLOOD;
}
...
conf_set_auth_mask(void *data)
{
	...
	if (yesno)
		yy_aconf->flags |= CONF_FLAGS_IPMASK;
	else
		yy_aconf->flags &= ~CONF_FLAGS_IPMASK;
}
...
conf_set_auth_mask_notice (void *data)
{
	...
	if (yesno)
		yy_aconf->flags |= CONF_FLAGS_MASK_NOTICE;
	else
		yy_aconf->flags &= ~CONF_FLAGS_MASK_NOTICE;
}
...
{
	static char noreason[] = "No Reason";
	...
	if(IsChannelName((char *) data))
	{
		if(resv_reason)
			...
			create_resv(data, noreason, RESV_CHANNEL);
	}
	else
	{
		conf_report_error("Warning -- channel '%s' in resv is not a valid channel name.",
				  (char *) data);
	}
}
...
{
	char noreason[] = "No Reason";
	if(clean_resv_nick(data))
	{
		if(resv_reason)
			create_resv(data, resv_reason, RESV_NICK);
		else
			create_resv(data, noreason, RESV_NICK);
	}
	else
	{
		conf_report_error("Warning -- nickname '%s' in resv is not a valid nickname.",
				  (char *) data);
	}
}
...
conf_set_shared_type(void *data)
{
	...
	yy_aconf->port = 0;
	set_modes_from_table(&yy_aconf->port, "flag", shared_table, args);
}
...
conf_begin_connect(struct TopConf *tc)
{
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, yy_hubleaf_list.head)
	{
		free_conf(ptr->data);
		dlinkDestroy(ptr, &yy_hubleaf_list);
	}
...
{
	if(inetpton(DEF_FAM, (char *) data, &IN_ADDR(yy_aconf->my_ipnum)) <= 0)
	{
...
conf_set_connect_rsa_public_key_file(void *data)
{
	...
	if(file == NULL)
	{
		conf_report_error("Ignoring connect::rsa_public_key_file %s -- couldn't open file.",
				  data);
		return;
	}
...
conf_set_connect_hub_mask(void *data)
{
	...
	{
		conf_report_error("Ignoring connect::hub_mask -- empty string.");
		return;
	}

	yy_tmp = make_conf();
	yy_tmp->status = CONF_HUB;
	DupString(yy_tmp->user, "*");
	DupString(yy_tmp->host, data);

	dlinkAddAlloc(yy_tmp, &yy_hubleaf_list);
}
...
conf_set_connect_leaf_mask(void *data)
{
	...
	if(EmptyString((const char *) data))
	{
		conf_report_error("Ignoring connect::leaf_mask -- empty string.");
		return;
	}

	yy_tmp = make_conf();
	yy_tmp->status = CONF_LEAF;
	DupString(yy_tmp->user, "*");
	DupString(yy_tmp->host, data);
...
conf_set_general_kline_delay(void *data)
{
	...
	/* note, this must be here to prevent us getting stuck in a
	 * a loop where we can never check klines --fl
	 */
	kline_queued = 0;
}
...
static void
conf_set_general_message_locale(void *data)
{
...
static void
conf_set_general_hidden_oper_confonly(void *data)
{
	ConfigFileEntry.hidden_oper_confonly = *(unsigned int *) data;
}
...
static void
conf_set_general_operstring_admin_only(void *data)
{
	ConfigFileEntry.operstring_admin_only = *(unsigned int *) data;
}
...
conf_set_general_default_operstring(void *data)
{
	...
	strlcpy(ConfigFileEntry.default_operstring, data, sizeof(ConfigFileEntry.default_operstring));
}
...
conf_set_general_default_adminstring(void *data)
{
	...
	strlcpy(ConfigFileEntry.default_adminstring, data, sizeof(ConfigFileEntry.default_adminstring));
}
...
conf_report_error(const char *fmt, ...)
{
	...
	ilog(L_ERROR, "\"%s\", line %d: %s", current_file, lineno + 1, msg);
...
newconf_init()
{
	...
	add_conf_item("logging", "fname_operspylog", CF_QSTRING,
		      conf_set_logging_fname_operspylog);
	...
	add_conf_item("operator", "local_kill", CF_YESNO, conf_set_oper_local_kill);
	...
	add_conf_item("operator", "hidden_oper", CF_YESNO, conf_set_oper_hidden_oper);
	...
	add_conf_item("general", "operstring_admin_only", CF_YESNO,
		      conf_set_general_operstring_admin_only);
	add_conf_item("general", "operspy_admin_only", CF_YESNO,
		      conf_set_general_operspy_admin_only);
	...
	add_conf_item("general", "message_locale", CF_QSTRING, conf_set_general_message_locale);
	...
	add_conf_item("general", "hidden_oper_confonly", CF_YESNO, conf_set_general_hidden_oper_confonly);
	...
	add_conf_item("general", "default_operstring", CF_QSTRING, conf_set_general_default_operstring);
	add_conf_item("general", "default_adminstring", CF_QSTRING, conf_set_general_default_adminstring);
