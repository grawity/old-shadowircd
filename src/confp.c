 ...
 *
 * $Id: confp.c,v 1.2 2004/05/26 15:13:39 nenolod Exp $
 */
...
config_parse(char *filename, char *confdata)
{
	...
	case '\"':
		...
		if (curce->ce_vardata) {
		    ...
		    curce->ce_vardatanum = strtol(curce->ce_vardata, &eptr, 0) & 0xffffffff;	/* we 
												 * only 
												 * want 
												 * 32bits 
												 * and 
    ...
    if (curce) {
	config_error
	    ("%s: Unexpected EOF for variable starting on line %i\n",
	     filename, curce->ce_varlinenum);
	config_entry_free(curce);
	config_free(curcf);
	return NULL;
	...
	config_error
	    ("%s: Unexpected EOF for section starting on line %i\n",
