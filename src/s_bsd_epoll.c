 ...
 *  USA
 *
 *  $Id: s_bsd_epoll.c,v 1.1 2004/07/29 15:27:05 nenolod Exp $
 */
...
#error "You must have a working sys/epoll.h"
#else
#include <sys/epoll.h>
#include <sys/syscall.h>
/* We'll use our own until glibc supports the a bit better */
...
comm_select(unsigned long delay)
{
	...
	struct epoll_event ep_event;
	static struct epoll_event pfd[HARD_FDLIMIT];
	void *data;

	num = epoll_wait(ep, pfd, HARD_FDLIMIT, delay);
	set_time();
	...
	for (i = 0; i < num; i++)
	{
		...
		if(pfd[i].events & (EPOLLIN | EPOLLHUP | EPOLLERR))
		{
			...
			if(hdl) {
				...
				ilog(L_WARN, "epoll.c: NULL read handler called");
		...
		if(pfd[i].events & (EPOLLOUT | EPOLLHUP | EPOLLERR))
		{
			...
			if(hdl) {
				...
				ilog(L_WARN, "epoll.c: NULL write handler called");
		}
