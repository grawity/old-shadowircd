  ...
  if (IsServer (to) && !(to->flags & FLAGS_CLOSING))
    {
      ...
      sendto_snomask (SNOMASK_NETINFO, fbuf, get_client_name (to, HIDEME), strerror (errtmp));
  ...
  if (!remote && IsPerson (from))
      ...
      if (user)
	{
	  ...
	  if (*user->host && !MyConnect (from))
	    {
	      buffer[sidx++] = '@';
	      for (p = (IsHidden (from) ? user->virthost : user->host); *p;
		   p++)
		buffer[sidx++] = *p;
	      flag = 1;
	    }
...
sendto_serv_butone (aClient * one, char *pattern, ...)
{
  ...
  for (i = serv_fdlist.entry[j = 1];
    ...
    {
      if (!(client_p = local[i]) || (one && client_p == one->from))
	continue;
 ...
 * we omit "one", too.
 * We however do NOT send to servers which are CLIENT capable regardless if they are NICKIP capable or not.
 */
void
sendto_nickip_servs_butone (int nickip, aClient * one, char *pattern, ...)
{
  int i;
  aClient *client_p;
  int j, k = 0;
  ...
  for (i = serv_fdlist.entry[j = 1];
    ...
    {
      if (!(client_p = local[i]) ||
	  (nickip && !IsNICKIP (client_p)) ||
	  (!nickip && IsNICKIP (client_p)) ||
	  (IsClientCapable (client_p)) || one == client_p)
	continue;
				 ...
				 char *pattern, ...)
{
  ...
  for (i = serv_fdlist.entry[j = 1];
    ...
    {
      if (!(client_p = local[i]) ||
	  (clientcapab && !IsClientCapable (client_p)) ||
...
send_quit_to_common_channels (aClient * from, char *reason)
{
  ...
  for (channels = from->user->channel; channels; channels = channels->next)
    {
      if (!can_send (from, channels->value.channel_p))
	{
 ...
 */
void
send_part_to_common_channels (aClient * from)
{
  Link *channels;
  chanMember *users;
  aClient *client_p;
  ...
  for (channels = from->user->channel; channels; channels = channels->next)
      ...
      if (can_send (from, channels->value.channel_p))
	{
	  msglen = sprintf (sendbuf, ":%s!%s@%s PART %s",
			    ...
			    (IsHidden (from) ? from->user->virthost : from->
			     user->host), channels->value.channel_p->chname);
 ...
 */
void
sendto_servs (aChannel * channel_p, aClient * from, char *pattern,
		      ...)
{
  int j, k = 0;
  fdlist send_fdlist;
  ...
  for (i = serv_fdlist.entry[j = 1]; j <= serv_fdlist.last_entry;
    ...
    {
      if (!(client_p = local[i]) || (client_p == from))
	continue;
		     ...
		     char *pattern, ...)
{
  ...
  if (MyConnect (from))
    {
      cansendlocal = ((from->oflag & OFLAG_LNOTICE)) ? 1 : 0;
      cansendglobal = ((from->oflag & OFLAG_GNOTICE)) ? 1 : 0;
    }
  else
...
void
sendto_snomask (int flag, char *pattern, ...)
{
  aClient *client_p;
  int i;
  char nbuf[1024];
...
sendto_ops (char *pattern, ...)
{
  ...
  for (i = 0; i <= highest_fd; i++)
    if ((client_p = local[i]) && !IsServer (client_p) && !IsMe (client_p))
      {
	(void) ircsprintf (nbuf, ":%s NOTICE %s :*** Server-Notice -- ",
			   me.name, client_p->name);
	(void) strncat (nbuf, pattern, sizeof (nbuf) - strlen (nbuf));
	vsendto_one (client_p, nbuf, vl);
      }
      ...
      if ((client_p = local[i]) != NULL)
	{
	  if (!
	      (IsRegistered (client_p)
	       && ((client_p->umode & UMODE_w) || IsServer (client_p)))
	      || client_p == one)
	    continue;
	  vsendto_prefix_one (client_p, from, pattern, vl);
	}
...
sendto_netglobal (char *pattern, ...)
{
  ...
  va_start (vl, pattern);
  for (i = 0; i <= highest_fd; i++)
    if ((client_p = local[i]) && !IsServer (client_p) && IsAnOper (client_p)
	&& !IsMe (client_p))
      {
	(void) ircsprintf (nbuf, ":%s NOTICE %s :*** \2Network-Global\2 -- ",
			   me.name, client_p->name);
	(void) strncat (nbuf, pattern, sizeof (nbuf) - strlen (nbuf));
	vsendto_one (client_p, nbuf, vl);
      }
...
flush_connections (int fd)
{
  ...
  if (fd == me.fd)
	...
	if ((client_p = local[i]) && !(client_p->flags & FLAGS_BLOCKED)
...
dump_connections (int fd)
{
  ...
  if (fd == me.fd)
    {
      for (i = highest_fd; i >= 0; i--)
	if ((client_p = local[i]) && DBufLength (&client_p->sendQ) > 0)
	  send_queued (client_p);
    }
  else if (fd >= 0 && (client_p = local[fd])
	   && DBufLength (&client_p->sendQ) > 0)
