...
_send_linebuf(struct Client *to, buf_head_t * linebuf)
{
	...
	if(IsDead(to) || to->localClient->fd == -1)
		return 0;
	...
	if(linebuf_len(&to->localClient->buf_sendq) > 0 && !IsDead(to))
...
send_queued_write(int fd, void *data)
{
	...
	if(linebuf_len(&to->localClient->buf_sendq) && !IsDead(to))
		      ...
		      struct Channel *chptr, const char *command, const char *pattern, ...)
{
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, chptr->members.head)
	{
		...
		if(target_p->from == one || IsDead(target_p))
			continue;
	...
	DLINK_FOREACH(ptr, global_serv_list.head)
	{
		...
		if(match(mask, target_p->name))
		{
			...
			if((cap && !IsCapable(target_p->from, cap)) ||
			   (nocap && !NotCapable(target_p->from, nocap)))
				continue;
