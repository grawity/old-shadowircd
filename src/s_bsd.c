 ...
 *  USA
 *
 *  $Id: s_bsd.c,v 1.2 2004/07/29 20:05:56 nenolod Exp $
 */
...
add_connection(struct Listener *listener, int fd)
{
	...
	inetntop(DEF_FAM, &IN_ADDR(new_client->localClient->ip), new_client->localClient->sockhost,
	    HOSTIPLEN);
#ifdef IPV6
	...
	strlcpy(new_client->host, new_client->localClient->sockhost, sizeof(new_client->host));
	...
	if((listener->is_ssl) && !(new_client->ssl)) {
		...
		if((new_client->ssl = SSL_new(ServerInfo.ctx)) == NULL) {
			...
			sendto_realops_flags(UMODE_DEBUG, L_ALL,
			    "Could not create new ssl connection %s :%s",
			    get_listener_name(listener), errno);
			ServerStats->is_ref++;
		...
		SetSSL(new_client);
		sendto_realops_flags(UMODE_DEBUG, L_ALL, "Created new ssl connection %s",
	...
	if(IsSSL(new_client) && new_ssl) {
		sendto_realops_flags(UMODE_DEBUG, L_ALL, "Accepting SSL connection %s",
		    get_listener_name(listener));
		SSL_set_fd(new_client->ssl, new_client->localClient->fd);
		if(!safe_SSL_accept(new_client, new_client->localClient->fd)) {
			dlink_node *m;

			sendto_realops_flags(UMODE_DEBUG, L_ALL, "Unsafe acception on %s",
			    get_listener_name(listener));
			m = dlinkFind(&unknown_list, new_client);
			if(m)
				dlinkDeleteNode(m, &unknown_list);
	...
	if(IsServer(client_p) || IsHandshake(client_p)) {
		...
		if(error == 0) {
			...
			sendto_realops_flags(UMODE_ALL, L_ADMIN,
			    "Server %s closed the connection", get_client_name(client_p, SHOW_IP));
...
comm_connect_tcp(int fd, const char *host, u_short port,
    struct sockaddr *clocal, int socklen, CNCB * callback, void *data, int aftype, int timeout)
{
...
comm_connect_tryconnect(int fd, void *notused)
{
	 ...
	 * Try the connect() 
	 */
	retval = connect(fd, (struct sockaddr *)&SOCKADDR(fd_table[fd].connect.hostaddr),
...
comm_tryssl_callback(int fd, void *data)
{
	...
	if(IsSSL(new_client)) {
		if(!SSL_is_init_finished(new_client->ssl)) {
			if(!safe_SSL_accept(new_client, new_client->localClient->fd)) {
				...
				sendto_realops_flags(UMODE_DEBUG, L_ALL, "Unsafe acception.");
				return;
			} else {
				comm_setselect(fd, fdlist, COMM_SELECT_WRITE, comm_tryssl_callback,
				    new_client, 0);
				return;
			}
		}
	...
	if(doauth == 1) {
		sendto_realops_flags(UMODE_DEBUG, L_ALL, "Starting Auth");
