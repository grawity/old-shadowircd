...
#ifdef USE_POLL
# ifdef __OpenBSD__		/* 2002/02/02 --tsk */
# undef isalpha
# undef isdigit
# undef isxdigit
# undef isalnum
...
report_error (char *text, aClient * client_p)
{
  ...
  if (!IsMe (client_p) && client_p->fd >= 0)
    if (!getsockopt (client_p->fd, SOL_SOCKET, SO_ERROR, (char *) &err, &len))
      if (err)
	errtmp = err;
#endif
  sendto_snomask (SNOMASK_DEBUG, text, host, strerror (errtmp));
#ifdef USE_SYSLOG
...
#endif
{
  ...
  if (getsockname (client_p->fd, (struct SOCKADDR *) &server, &len))
    {
...
const char* get_listener_name(struct Client * client_p)
{
  static char buf[HOSTLEN + HOSTLEN + 10];
  ircsprintf(buf, "%s[%s/%u]", me.name, client_p->acpt->name, 
             client_p->acpt->port);
  return buf;
}
...
init_sys ()
{
  ...
  if (!getrlimit (RLIMIT_FD_MAX, &limit))
	...
	{
	  (void) fprintf (stderr, "§ Error fd table too big\n");
...
# endif
  ...
  printf ("Initiailizing resolver (this may take a while)... ");
...
check_init (aClient * client_p, char *sockn)
{
  ...
  if (getpeername (client_p->fd, (struct SOCKADDR *) &sk, &len) == -1)
    {
  ...
  if (IsConnecting (client_p) || IsHandshake (client_p))
    {
      ...
      if (!c_conf || !n_conf)
	{
	  sendto_snomask (SNOMASK_DEBUG, "Connecting Error: %s[%s]", name,
			      client_p->sockhost);
...
completed_connection (aClient * client_p)
{
  ...
  if (!aconf)
    {
      sendto_realops ("Lost C-Line for %s",
		      get_client_name (client_p, HIDEME));
      return -1;
    }
  ...
  if (!nconf)
    {
      sendto_realops ("Lost N-Line for %s",
		      get_client_name (client_p, HIDEME));
      return -1;
    }
...
close_connection (aClient * client_p)
{
  ...
  if ((aconf = find_conf_exact (client_p->name, client_p->username,
				client_p->sockhost, CONF_CONNECT_SERVER)))
    {
      /* 
...
set_non_blocking (int fd, aClient * client_p)
{
  ...
  if (ioctl (fd, FIONBIO, &res) < 0)
    silent_report_error ("ioctl(fd,FIONBIO) failed for %s:%s", client_p);
#else
  if ((res = fcntl (fd, F_GETFL, 0)) == -1)
    silent_report_error ("fcntl(fd, F_GETFL) failed for %s:%s", client_p);
  else if (fcntl (fd, F_SETFL, res | nonb) == -1)
    silent_report_error ("fcntl(fd, F_SETL, nonb) failed for %s:%s",
			 client_p);
#endif
  return;
}
...
add_connection (aClient * client_p, int fd)
{
  ...
  memcpy ((char *) &target_p->ip, (char *) &addr.SIN_ADDR,
	  sizeof (struct IN_ADDR));
   ...
   */
  for (s = (char *) &client_p->ip, t = (char *) &target_p->ip, len = IPLEN;
       len > 0; len--, s++, t++)
    {
      if (!*s)
	continue;
  ...
  if (IsSSL (client_p))
	...
	{
	  sendto_snomask (SNOMASK_DEBUG, "SSL creation of "
  ...
  if (!find_matching_conf (&EList1, target_p->hostip))
      ...
      if (ban)
	{
	  ...
	  sendto_one (target_p, "NOTICE * :*** %s", ktype);
...
#endif
	  exit_client (target_p, target_p, &me, ktype);
	  return NULL;
	}
    }
  ...
  if (!IsSSL (target_p))
    {
      sendto_one (target_p, "NOTICE * :*** Please wait while your connection is processed.");
      sendto_one (target_p, "NOTICE * :*** If you have any problems connecting to %s, please contact %s.",
	me.name, SERVER_KLINE_ADDRESS);
    }
#endif
...
do_client_queue (aClient * client_p)
{
  ...
  while (DBufLength (&client_p->recvQ) && !NoNewLine (client_p) &&
...
#if defined(MAXBUFFERS)
	  dolen =
	    dbuf_get (&client_p->recvQ, readbuf, rcvbufmax * sizeof (char));
#else
  ...
  if (IsServer (client_p) || IsConnecting (client_p) || IsHandshake (client_p)
	  ...
	  DBufLength (&client_p->recvQ) > CLIENT_FLOOD)
	{
	  sendto_snomask (SNOMASK_FLOOD,
			      ...
			      client_p->user ? client_p->user->username : "*",
...
accept_connection (aClient * client_p)
{
  ...
  for (i = 0; i < 100; i++)	/* accept up to 100 times per call to deal with high connect rates */
    {
      if ((newfd =
	   accept (client_p->fd, (struct SOCKADDR *) &addr, &addrlen)) < 0)
	{
	  switch (errno)
	    {
#ifdef EMFILE
...
/* if they are throttled, drop them silently. */
	...
	{
	  ircstp->is_ref++;
	  sendto_snomask (SNOMASK_CONNECTS,
			      "All connections in use. fd: %d (%s)", newfd,
			      get_client_name (client_p, HIDEME));
...
#ifndef USE_POLL
int
read_message (time_t delay, fdlist * listp)
{
  aClient *client_p;
  int nfds;
  struct timeval wait;
  ...
  for (res = 0;;)
	   ...
	   i = listp->entry[++j])
	{
...
#ifdef USE_SSL
	  if (client_p->ssl != NULL && IsSSL (client_p) &&
	      !SSL_is_init_finished (client_p->ssl))
	    {
	      if (IsDead (client_p)
		  || (!safe_SSL_accept (client_p, client_p->fd)))
		close_connection (client_p);
	      continue;
	    }
#endif
	  ...
	  if (DoingAuth (client_p))
	    {
	      auth++;
	      Debug ((DEBUG_NOTICE, "auth on %x %d", client_p, i));
	      FD_SET (client_p->authfd, read_set);
	      if (client_p->flags & FLAGS_WRAUTH)
		FD_SET (client_p->authfd, write_set);
	    }
	  if (DoingDNS (client_p) || DoingAuth (client_p))
	    continue;
	  if ((IsMe (client_p) && IsListening (client_p))
	      || IsConnecting (client_p))
	    {
	      FD_SET (i, read_set);
	    }
	  else if (!IsMe (client_p))
	    {
	      if (DBufLength (&client_p->recvQ) && delay2 > 2)
		delay2 = 1;

	      FD_SET (i, read_set);
	    }

	  length = DBufLength (&client_p->sendQ);

	  if (DoList (client_p) && IsSendable (client_p))
	    {
	      send_list (client_p, 64);
	      length = DBufLength (&client_p->sendQ);
	    }

	  if (length || IsConnecting (client_p) ||
	      (ZipOut (client_p)
	       && zip_is_data_out (client_p->serv->zip_out)))
	    FD_SET (i, write_set);
	}
...
# endif
      ...
      if (MYFD_ISSET_WRITE)
	{
	  ...
	  if (IsConnecting (client_p))
	    write_err = completed_connection (client_p);
	  if (!write_err)
	    send_queued (client_p);

	  if (IsDead (client_p) || write_err)
	    {
	      if (MYFD_ISSET_READ)
		{
		MYFD_CLR_READ}
      ...
      if (MYFD_ISSET_READ)
	length = read_packet (client_p);
      else if (DBufLength (&client_p->recvQ) && IsPerson (client_p)
	       && !NoNewLine (client_p))
	length = do_client_queue (client_p);

# ifdef DEBUGMODE
      readcalls++;
# endif

      if ((length != FLUSH_BUFFER) && IsDead (client_p))
	{
	  if (MYFD_ISSET_READ)
	    {
	    MYFD_CLR_READ}
	   ...
	   i = listp->entry[++j])
	{
	  ...
	  if (DoingAuth (client_p))
		...
		memset ((char *) &authclnts, '\0', sizeof (authclnts));
	      auth++;
	      Debug ((DEBUG_NOTICE, "auth on %x %d", client_p, i));
	      PFD_SETR (client_p->authfd);
	      if (client_p->flags & FLAGS_WRAUTH)
		PFD_SETW (client_p->authfd);
	      authclnts[client_p->authfd] = client_p;
	      continue;
	    }
...
# ifdef CONNECTFAST
	      ...
	      PFD_SETR (i);
# else
	      if (timeofday > (client_p->lasttime + 2))
		{
		  PFD_SETR (i);
		}
	      else if (delay2 > 2)
...
# endif
  ...
  for (pfd = poll_fdarray, i = 0; i < nbr_pfds; i++, pfd++)
      ...
      else			/* nfds == 0 or there are no events for this socket */
	{
	  ...
	  if (DBufLength (&client_p->recvQ) && IsPerson (client_p)
	      && !NoNewLine (client_p))
	    length = do_client_queue (client_p);
	  else
	    continue;
...
# endif
	...
	continue;
      ...
      /* An error has occured reading from client_p, drop it. */
      read_error_exit (client_p, length, client_p->sockerr);
    }
  return 0;
}
...
connect_server (aConfItem * aconf, aClient * by, struct hostent *hp)
{
   ...
   */
  (void) attach_confs_host (client_p, aconf->host,
			    CONF_NOCONNECT_SERVER | CONF_CONNECT_SERVER);

  if (!find_conf_host (client_p->confs, aconf->host, CONF_NOCONNECT_SERVER) ||
...
connect_inet (aConfItem * aconf, aClient * client_p, int *lenp)
{
  ...
  if (aconf->localhost)
     ...
     */
    inet_pton (AFINET, (void *) aconf->localhost,
  ...
  if (specific_virtual_host || aconf->localhost)
    {
       ...
       * --smg
       */
      if (bind (client_p->fd, (struct SOCKADDR *) &sin, sizeof (sin)) == -1)
	{
  ...
  memcpy ((char *) &client_p->ip, (char *) &aconf->ipnum,
	  sizeof (struct IN_ADDR));
