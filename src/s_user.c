...
register_local_user(struct Client *client_p, struct Client *source_p, char *nick, char *username)
{
	...
	if(!IsGotId(source_p)) {
		...
		/*
		 * Don't replace username if its supposed to be spoofed 
		 */
	...
	if(((Count.local + 1) >= (GlobalSetOptions.maxclients + MAX_BUFFER)
		...
		sendto_realops_flags(UMODE_FULL, L_ALL, "Too many clients, rejecting %s[%s].", nick,
		    source_p->host);
	...
	if(!valid_username(source_p->username)) {
		sendto_realops_flags(UMODE_REJ, L_ALL,
		    "Invalid username: %s (%s@%s)", nick, source_p->username, source_p->host);
		ServerStats->is_ref++;
	 ...
	 */
	if(!IsExemptKline(source_p) && ((status = check_X_line(client_p, source_p)) < 0))
		return status;
...
register_remote_user(struct Client *client_p, struct Client *source_p, char *nick, char *username)
{
	...
	if((target_p = find_server(user->server)) && target_p->from != source_p->from) {
		sendto_realops_flags(UMODE_DEBUG, L_ALL,
		    ...
		    source_p->host, user->server, target_p->name, target_p->from->name);
...
introduce_client(struct Client *client_p, struct Client *source_p, struct User *user, char *nick)
{
	...
	sendto_server(client_p, NULL, NOCAPS, CAP_IPMASK,
	    ...
	    source_p->username, source_p->host, user->server, source_p->info);

	sendto_server(client_p, NULL, CAP_IPMASK, NOCAPS,
	    "NICK %s %d %lu %s %s %s %s %s :%s",
	    nick, source_p->hopcount + 1,
	    (unsigned long)source_p->tsinfo, ubuf,
	    source_p->username, source_p->host, user->server, source_p->rhost, source_p->info);
...
report_and_set_user_flags(struct Client *source_p, struct ConfItem *aconf)
{
	...
	if(IsConfDoSpoofIp(aconf)) {
		sendto_one(source_p,
		    ":%s NOTICE %s :*** Spoofing your IP. congrats.", me.name, source_p->name);
	}
...
user_mode(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(IsServer(source_p) && !MyClient(target_p))
		sendto_server(source_p, NULL, NOCAPS, NOCAPS,
			      ":%s MODE %s %s", parv[0], parv[1], parv[2]);


	/*
	 * Did some more enhancing here for admins. Now they can view umodes with /mode <user>.
	 */
	if(parc < 3) {
		m = buf;
		*m++ = '+';
	...
	/*
	 * Admins can't *change* usermodes though.
	 */
		...
		for (m = *p; *m; m++)
			...
			case 'X':
				if(MyClient(target_p)) {
					aconf =
					    find_address_conf(target_p->rhost, target_p->username,
					    &target_p->localClient->ip,
					    target_p->localClient->aftype);

					strcpy(oldhost, target_p->host);

					if(what == MODE_ADD) {
						if(IsConfDoSpoofIp(aconf)) {
							target_p->umodes &= ~UMODE_IPMASK;
							break;
						}

						if(IsMasked(target_p)) {
							if(!strcmp(ret_maskip(target_p),
								target_p->host))
								break;

							target_p->umodes |= UMODE_IPMASK;
							strcpy(target_p->host,
							    ret_maskip(target_p));
						}

						target_p->umodes |= UMODE_IPMASK;
						strcpy(target_p->host, ret_maskip(target_p));
					}
					...
					if(what == MODE_DEL) {
						...
						target_p->umodes &= ~UMODE_IPMASK;
						strcpy(target_p->host, target_p->rhost);
					}

					propagate_generic(&me, "SVSHOST", "*", CAP_IPMASK, "%s :%s",
			...
			case 'p':
				if(MyConnect(target_p)) {
					if(ConfigFileEntry.hidden_oper_confonly
					    || !IsOper(target_p)) {
						badflag = YES;
					} else {
						if(what == MODE_ADD)
							target_p->umodes |= UMODE_HIDDENOPER;
						else
							target_p->umodes &= ~UMODE_HIDDENOPER;
					}
				}
	...
	if(MyConnect(target_p) && (target_p->umodes & UMODE_ADMIN) &&
	    (!IsOperAdmin(target_p) || IsOperHiddenAdmin(target_p))) {
		sendto_one(target_p, ":%s NOTICE %s :*** You need oper and A flag for +a",
		    me.name, parv[0]);
...
user_welcome(struct Client *source_p)
{
	...
	if(ConfigFileEntry.short_motd) {
		...
		sendto_one(source_p,
		    form_str(RPL_MOTD), me.name, source_p->name, "*** This is the short motd ***");
...
oper_up(struct Client *source_p, struct ConfItem *aconf)
{
	...
	if(ConfigFileEntry.oper_umodes)
		...
		source_p->umodes |=
		    (UMODE_SERVNOTICE | UMODE_OPERWALL | UMODE_WALLOP | UMODE_LOCOPS) & ALL_UMODES;
	...
	sendto_realops_flags(UMODE_ALL, L_ALL,
	    "%s (%s@%s) is now an operator", source_p->name, source_p->username, source_p->host);
