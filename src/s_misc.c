 ...
 *  USA
 *
 *  $Id: s_misc.c,v 1.1 2003/11/04 07:11:34 nenolod Exp $
 *
 */
...
#if !defined(ULTRIX) && !defined(SGI) && !defined(sequent) && !defined(__convex__)
#include <sys/param.h>
#endif
#if defined(AIX) || defined(SVR3) || ((__GNU_LIBRARY__ == 6) && (__GLIBC__ >=2) && (__GLIBC_MINOR__ >= 2))
#include <time.h>
#endif
#include "h.h"
	  ...
	  if ((*lpp)->value.client_p == source_p)
	      ...
	      if ((*lpp)->flags == DCC_LINK_ME)
		{
		  sendto_one (target_p,
			      ...
			      me.name, RPL_DCCINFO, target_p->name,
			      source_p->name);
		}
	      tmp = *lpp;
	      *lpp = tmp->next;
  ...
  if (!IsULine (client_p))
    {
      ...
      sendto_common_channels (client_p, ":%s QUIT :%s", client_p->name,
			      reason);
    }
  else
    {
		 ...
		 aClient * lclient_p, char *spinfo, char *comment)
{
  ...
  for (i = serv_fdlist.entry[j = 1]; j <= serv_fdlist.last_entry;
    ...
    {
      if (!(target_p = local[i]) || target_p == client_p || IsMe (target_p)
 ...
 * 
 *      FLUSH_BUFFER    if (client_p == source_p) *     0               if
 * (client_p != source_p)
 */
int
exit_client (aClient * client_p,	/*
  ...
  )
{
  ...
  if (MyConnect (source_p))
      ...
      if (IsServer (source_p))
	{
	  ...
	  if (IsULine (source_p))
	    Count.myulined--;
	  delfrom_fdlist (source_p->fd, &serv_fdlist);
#ifdef NO_CHANOPS_WHEN_SPLIT
      ...
      if (IsPerson (source_p))
	{
	  ...
	  sendto_snomask
	    (SNOMASK_CONNECTS, "Client exiting at %s: %s (%s@%s) [%s]%s [%s]", me.name,
...
# endif
	  ...
	  (void) strcat (comment1, source_p->name);
	  for (target_p = client; target_p; target_p = next)
	    {
	      next = target_p->next;
	      if (!IsServer (target_p) && target_p->from == source_p)
		exit_one_client (source_p, target_p, &me, comment1);
	    }
	  /*
	  ...
	  for (target_p = client; target_p; target_p = next)
	    {
	      next = target_p->next;
	      if (IsServer (target_p) && target_p->from == source_p)
		exit_one_client (source_p, target_p, &me, me.name);
	    }
# ifdef ALWAYS_SEND_DURING_SPLIT
...
# endif
      ...
      for (i = 0; i <= highest_fd; i++)
	{
	  ...
	  if ((aconf = target_p->serv->nline) &&
	      (match (my_name_for_link (me.name, aconf), source_p->name) ==
	       0))
	    continue;
	  /*
	  ...
	  if (source_p->from == target_p)
	    ...
	    {
	      sendto_one (target_p, "SQUIT %s :%s", source_p->name, comment);
	    }
	}
#endif /* USE_NOQUIT */
    }
	...
	{
	  sendto_serv_butone (client_p, ":%s QUIT :%s", source_p->name,
			      comment);
	}
      /*
      ...
      if (source_p->user)
	{
	  ...
	  send_part_to_common_channels (source_p);
  ...
  for (fd = l.entry[j = 1]; j <= l.last_entry; fd = l.entry[++j])
      ...
      if (ZipOut (target_p))
	{
	  ...
	  if (ib)
	    {
	      sendto_one (client_p,
			  ":%s %d %s : - Zip inbytes %d, outbytes %d (%3.2f%%)",
...
show_opers (aClient * client_p, char *name)
{
  ...
  for (fd = l->entry[i = 1]; i <= l->last_entry; fd = l->entry[++i])
    {
      if (!(client_p2 = local[fd]))
	continue;
      if (!IsClient (client_p2))
	{
	  sendto_one (client_p,
		      ":%s %d %s :Weird fd %d(%d) - not client (%d)", me.name,
		      RPL_STATSDEBUG, name, fd, client_p2->fd,
		      client_p2->status);
	  continue;
	}
      if (!IsAnOper (client_p2))
	{
	  delfrom_fdlist (client_p2->fd, &oper_fdlist);
	  continue;
	}
  ...
  for (fd = l->entry[i = 1]; i <= l->last_entry; fd = l->entry[++i])
    {
      if (!(client_p2 = local[fd]))
	continue;
      if (!IsServer (client_p2))
	{
	  sendto_one (client_p,
		      ":%s %d %s :Weird fd %d(%d) - not server (%d)", me.name,
		      RPL_STATSDEBUG, name, fd, client_p2->fd,
		      client_p2->status);
	  continue;
	}
