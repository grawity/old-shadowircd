/*
 *  UltimateIRCd - an Internet Relay Chat Daemon, src/res.c
 *
 ...
 *  USA
 *
 *  $Id: res.c,v 1.1 2003/11/04 07:11:10 nenolod Exp $
 *
 */
...
static void
rem_request_id (ResRQ * req)
{
  unsigned int hv = hash_id (req->id);
  ResRQ *rptr, *r2ptr = NULL;

  for (rptr = idcphashtable[hv].id_list; rptr;
       r2ptr = rptr, rptr = rptr->id_hashnext)
    {
      if (rptr != req)
	continue;
...
static void
rem_request_cp (ResRQ * req)
{
  unsigned int hv = hash_cp (req->cinfo.value.cp);
  ResRQ *rptr, *r2ptr = NULL;

  for (rptr = idcphashtable[hv].cp_list; rptr;
       r2ptr = rptr, rptr = rptr->cp_hashnext)
    {
      if (rptr != req)
	continue;
  ...
  for (rptr = first; rptr; rptr = r2ptr)
      ...
      if (now >= tout)
	{
	  if (--rptr->retries <= 0)
	    {
#ifdef DEBUG
	      Debug ((DEBUG_ERROR, "timeout %x now %d client_p %x",
		      rptr, now, rptr->cinfo.value.client_p));
#endif
	      reinfo.re_timeouts++;
	      client_p = rptr->cinfo.value.client_p;
	  ...
	  else
	    {
	      ...
	      Debug ((DEBUG_INFO, "r %x now %d retry %d c %x",
		      rptr, now, rptr->retries, rptr->cinfo.value.client_p));
#endif
	    }
	}
      if (!next || tout < next)
  ...
  for (i = 0; i < max; i++)
    {
      ...
      if (sendto
	  (resfd, msg, len, 0, (struct sockaddr *) &(_res.nsaddr_list[i]),
...
struct hostent *
gethost_byname_type (char *name, Link * lp, int type)
{
  aCache *cp;
...
static int
do_query_name (Link * lp, char *name, ResRQ * rptr, int type)
{
...
#ifndef INET6
static int
do_query_number (Link * lp, struct in_addr *numb, ResRQ * rptr)
#else
static int
do_query_number (Link * lp, struct IN_ADDR *numb, ResRQ * rptr)
#endif
 ...
 */
static int
query_name (char *name, int class, int type, ResRQ * rptr)
{
  struct timeval tv;
  char buf[MAXPACKET];
  int r, s, k = 0;
...
#ifndef INET6
int
arpa_to_ip (char *arpastring, unsigned int *saddr)
{
  int idx = 0, onum = 0;
  ...
  if (irccmp (ipbuf + idx, "in-addr.arpa") != 0)
    return 0;
...
#else
int
arpa_to_ip (const char *arpastring, u_char * sa, size_t size)
{
  ...
  char *ip6 = "ip6.int", *ip4 = "in-addr.arpa", *str = NULL;
  ...
  if (strstr (arpastring, ip6))
    {
      /* ipbuf should contain a string like f.e.d.c.b.a.9.8.7.6.5.4.3.2.1.f.e.d.c.b.a.9.8.7.6.5.4.3.2.1.ip6.int */
      str = ip6;
      dots = 64;
...
static inline char *
is_acceptable_answer (char *h)
{
  int i;

  for (i = 0; i < num_acc_answers; i++)
    {
      if (irccmp (acceptable_answers[i], h) == 0)
 ...
 */
static int
proc_answer (ResRQ * rptr, HEADER * hptr, char *buf, char *eob)
{
  char *cp, **alias, *acc;
  struct hent *hp;
  ...
  if ((n = dn_expand (buf, eob, cp, hostbuf, sizeof (hostbuf))) <= 0)
	...
	{
	  sendto_snomask (SNOMASK_DEBUG,
			      "rptr->type is unknown type %d! (rptr->name == %x)",
			      rptr->type, rptr->name);
	  return -1;
	}

      if (irccmp (tmphost, hostbuf) != 0)
	{
	  sendto_snomask (SNOMASK_DEBUG,
			      "Asked question for %s, but got reply about question %s (!!!)",
			      tmphost, hostbuf);
	  strangeness++;
	}
  ...
  while (hptr->ancount-- > 0 && cp && cp < eob)
      ...
      if (!strchr (hostbuf, '.') && (_res.options & RES_DEFNAMES))
	{
	  ...
	  if ((len + 2) < sizeof (hostbuf))
	    {
	      strncpyzt (hostbuf, _res.defdname, sizeof (hostbuf) - 1 - len);
	      hostbuf[RESHOSTLEN] = '\0';
	...
	case T_A:
	  if (rptr->name == NULL)
	    {
	      sendto_snomask (SNOMASK_DEBUG,
				  "Received DNS_A answer, but null rptr->name!");
	      return PROCANSWER_STRANGE;
	    }
	  if (irccmp (rptr->name, hostbuf) != 0)
	    {
	      if (!num_acc_answers || !(acc = is_acceptable_answer (hostbuf)))
		{
#ifdef DNS_ANS_DEBUG
		  sendto_snomask (SNOMASK_DEBUG,
				      "Received DNS_A answer for %s, but asked question for %s",
				      hostbuf, rptr->name);
#endif
		  return PROCANSWER_STRANGE;
		}
	  ...
	  if (dlen !=
	      (type == T_AAAA ? sizeof (dr) : sizeof (struct in_addr)))
	    {
	      sendto_realops ("Bad IP length (%d) returned for %s", dlen,
			      hostbuf);
	      Debug ((DEBUG_DNS, "Bad IP length (%d) returned for %s", dlen,
		      hostbuf));
...
#else
	      if (type == T_AAAA)
		memcpy (hp->h_addr_list[adr].S_ADDR, dr.S_ADDR,
			sizeof (struct IN_ADDR));
	      else
		{
		  memset ((char *) hp->h_addr_list[adr].S_ADDR, 0x0, 10);
		  hp->h_addr_list[adr].S_ADDR[10] =
		    hp->h_addr_list[adr].S_ADDR[11] = 0xff;
		  memcpy (((char *) hp->h_addr_list[adr].S_ADDR) + 12,
			  dr.S_ADDR, 4);
		}
#endif
...
#ifdef DNS_ANS_DEBUG_MAX
	      sendto_snomask (SNOMASK_DEBUG, "%s A %s", dhostbuf,
				  inet_ntop (AFINET, &hp->h_addr_list[adr],
	  ...
	  if (!num_acc_answers || !(acc = is_acceptable_answer (hostbuf)))
	    {
	      ...
	      if (!
		  (arpa_to_ip
		   (hostbuf, ptrrep.S_ADDR, sizeof (ptrrep.S_ADDR))))
#endif
		{
#ifdef DNS_ANS_DEBUG
		  inet_ntop (AFINET, &ptrrep.S_ADDR, mydummy,
			     sizeof (mydummy));
		  inet_ntop (AFINET, &rptr->addr, mydummy2,
			     sizeof (mydummy2));
		  sendto_snomask (SNOMASK_DEBUG,
...
#else
	      if (memcmp
		  (ptrrep.S_ADDR, rptr->addr.S_ADDR, sizeof (struct IN_ADDR)))
#endif
		{
#ifdef DNS_ANS_DEBUG
		  inet_ntop (AFINET, (char *) &ptrrep.S_ADDR, mydummy,
			     sizeof (mydummy));
		  inet_ntop (AFINET, (char *) &rptr->addr, mydummy2,
			     sizeof (mydummy2));
		  sendto_snomask (SNOMASK_DEBUG,
				      "Received DNS_PTR answer for %s, "
				      "but asked question for %s", mydummy,
				      mydummy2);
#endif
		  return PROCANSWER_STRANGE;
		}
...
#ifdef DNS_ANS_DEBUG
	  if (acc)
	    sendto_snomask (SNOMASK_DEBUG, "DNS_PTR from an acceptable (%s)",
				acc);
#endif
...
#ifdef DNS_ANS_DEBUG_MAX
	  sendto_snomask (SNOMASK_DEBUG, "%s PTR %s", dhostbuf, hostbuf);
#endif
	...
	case T_CNAME:
	  ...
	  if (origtype == T_PTR)
	      ...
	      if (!num_acc_answers || !(acc = is_acceptable_answer (hostbuf)))
		{
...
#else
		  if (memcmp
		      (ptrrep.S_ADDR, rptr->addr.S_ADDR,
		       sizeof (struct IN_ADDR)))
#endif
		    {
#ifdef DNS_ANS_DEBUG
	      ...
	      if (acc)
		sendto_snomask (SNOMASK_DEBUG,
				    "DNS_CNAME (PTR) answer from an acceptable (%s)",
				    acc);
#endif
	    }
#ifndef INET6
		...
		{
		  if (!num_acc_answers
		      || !(acc = is_acceptable_answer (hostbuf)))
		    {
#ifdef DNS_ANS_DEBUG
		      sendto_snomask (SNOMASK_DEBUG,
					  "Received DNS_CNAME(A) answer for %s, but asked question for %s",
					  hostbuf, rptr->name);
#endif
		      return PROCANSWER_STRANGE;
		    }
#ifdef DNS_ANS_DEBUG
		  sendto_snomask (SNOMASK_DEBUG,
				      "DNS_CNAME (A) answer from an acceptable (%s)",
				      acc);
#endif
		}
	    }
...
#ifdef DNS_ANS_DEBUG_MAX
	  sendto_snomask (SNOMASK_DEBUG, "%s CNAME %s", dhostbuf, hostbuf);
#endif
...
get_res (char *lp)
{
  ...
  if (a > 0 && rptr->type == T_PTR)
    {
      ...
      Debug ((DEBUG_DNS, "relookup %s <-> %s",
	      rptr->he.h_name, inet_ntop (AFINET, &rptr->he.h_addr, mydummy,
					  sizeof (mydummy))));
  ...
  if (a > 0 && (rptr->type == T_AAAA || rptr->type == T_A))
	...
	{
	  sendto_snomask (SNOMASK_DEBUG,
			      "Blindly accepting dns result for %s",
			      rptr->he.h_name ? rptr->he.
			      h_name : inet_ntop (AFINET,
						  (char *) &rptr->addr,
						  mydummy, sizeof (mydummy)));
	}
      else
	  ...
	  if (invalid_parms_name || invalid_parms_ip)
	    {
	      sendto_snomask (SNOMASK_DEBUG,
				  ...
				  invalid_parms_name ? "MISSING" : rptr->he.
				  h_name,
				  invalid_parms_ip ? "MISSING" :
				  inet_ntop (AFINET,
					     (char *) &rptr->he.
					     h_addr_list[0], mydummy,
					     sizeof (mydummy)));
	  ...
	  for (numaddr = numnewaddr = nidx = 0; nidx < IRC_MAXADDRS; nidx++)
...
#else
		  if (!memcmp (rptr->he_rev.h_addr_list[tidx].S_ADDR, rptr->he.h_addr_list[nidx].S_ADDR, sizeof (struct IN_ADDR)))	/* MATCH */
#endif
		    {
		      found_match_ip++;
		      does_match = 1;
	      ...
	      if (does_match)
		{
		  ...
		  memcpy (new_addr_list[numnewaddr++].S_ADDR,
			  rptr->he.h_addr_list[nidx].S_ADDR,
			  sizeof (struct IN_ADDR));
		  memset (new_addr_list[numnewaddr].S_ADDR, 0x0,
			  sizeof (struct IN_ADDR));
#endif
		}
	    }
	  ...
	  if (!found_match_ip)
	    {
	      ...
	      inet_ntop (AFINET, &rptr->he.h_addr_list[0], ntoatmp_f,
			 sizeof (ntoatmp_f));
	      inet_ntop (AFINET, &rptr->he_rev.h_addr_list[0], ntoatmp_r,
			 sizeof (ntoatmp_r));

#ifdef DNS_ANS_DEBUG
	      sendto_snomask (SNOMASK_DEBUG,
				  "Forward and Reverse queries do not have matching IP! %s<>%s %s<>%s",
				  rptr->he.h_name, rptr->he_rev.h_name,
				  ntoatmp_f, ntoatmp_r);
#endif
	      if (rptr->cinfo.flags == ASYNC_CLIENT
		  && rptr->cinfo.value.client_p)
		{
		  sendto_one (rptr->cinfo.value.client_p,
...
#ifdef DNS_ANS_DEBUG
	      sendto_snomask (SNOMASK_DEBUG, "numaddr = %d, numnewaddr = %d",
				  numaddr, numnewaddr);
#endif
	    }
...
update_list (ResRQ * rptr, aCache * cachep)
{
  ...
  *cpp = cp->list_next;
  ...
  for (s = (char *) &rptr->he.h_addr.S_ADDR;
       WHOSTENTP (((struct IN_ADDR *) s)->S_ADDR);
      ...
      if (!t)
	{
	  ...
	  addrcount++;
	  t = (char *) MyRealloc (*base, addrcount * sizeof (struct IN_ADDR));
...
make_cache (ResRQ * rptr)
{
  ...
  for (i = 1; WHOSTENTP (rptr->he.h_addr_list[i].S_ADDR) && i < IRC_MAXADDRS;
       i++)
    if ((cp =
...
rem_cache (aCache * ocp)
{
  ...
  for (hashv = highest_fd; hashv >= 0; hashv--)
    if ((client_p = local[hashv]) && (client_p->hostp == hp))
      client_p->hostp = NULL;
  /*
...
int
m_dns (aClient * client_p, aClient * source_p, int parc, char *parv[])
{
  aCache *cp;
  int i;
  ...
  if (parv[1] && *parv[1] == 'l')
    {
      for (cp = cachetop; cp; cp = cp->list_next)
	{
	  sendto_one (source_p, "NOTICE %s :Ex %d ttl %d host %s(%s)",
		      parv[0], cp->expireat - timeofday, cp->ttl,
		      cp->he.h_name, inet_ntop (AFINET, cp->he.h_addr,
						mydummy, sizeof (mydummy)));
	  for (i = 0; cp->he.h_aliases[i]; i++)
	    sendto_one (source_p, "NOTICE %s : %s = %s (CN)",
			parv[0], cp->he.h_name, cp->he.h_aliases[i]);
	  for (i = 1; cp->he.h_addr_list[i]; i++)
	    sendto_one (source_p, "NOTICE %s : %s = %s (IP)",
  ...
  sendto_one (source_p, "NOTICE %s :Ca %d Cd %d Ce %d Cl %d Ch %d:%d Cu %d",
	      source_p->name,
	      cainfo.ca_adds, cainfo.ca_dels, cainfo.ca_expires,
	      cainfo.ca_lookups,
	      cainfo.ca_na_hits, cainfo.ca_nu_hits, cainfo.ca_updates);

  sendto_one (source_p, "NOTICE %s :Re %d Rl %d/%d Rp %d Rq %d",
	      source_p->name, reinfo.re_errors, reinfo.re_nu_look,
	      reinfo.re_na_look, reinfo.re_replies, reinfo.re_requests);
  sendto_one (source_p, "NOTICE %s :Ru %d Rsh %d Rs %d(%d) Rt %d",
	      source_p->name, reinfo.re_unkrep, reinfo.re_shortttl,
	      reinfo.re_sent, reinfo.re_resends, reinfo.re_timeouts);
  return 0;
}

u_long
cres_mem (aClient * source_p)
{
  aCache *c = cachetop;
  struct hostent *h;
  int i;
  ...
  sendto_one (source_p, ":%s %d %s :RES table sz %d",
	      me.name, RPL_STATSDEBUG, source_p->name, ts);
  sendto_one (source_p,
	      ":%s %d %s :RES Structs sz %d IP storage sz %d Name storage sz %d",
	      me.name, RPL_STATSDEBUG, source_p->name, sm, im, nm);
  return ts + sm + im + nm;
}
