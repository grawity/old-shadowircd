 ...
 *  USA
 *
 *  $Id: adns.c,v 1.1 2004/07/29 15:27:29 nenolod Exp $
 */
...
static void dns_do_callbacks(void);
static void dns_readable(int fd, void *ptr);
static void dns_select(void);
...
dns_select(void)
{
	...
	for (i = 0; i < npollfds; i++)
	{
		...
		if(pollfds[i].events & ADNS_POLLOUT)
			comm_setselect(fd, FDLIST_SERVICE, COMM_SELECT_WRITE, dns_writeable, NULL,
				       0);
	}
...
adns_getaddr(struct irc_inaddr *addr, int aftype, struct DNSQuery *req, int arpa_type)
{
	...
	if(aftype == AF_INET6)
	{
		...
		if(!arpa_type)
		{
			result = adns_submit_reverse(dns_state, (struct sockaddr *) &ipn.sins.sin6,
					    adns_r_ptr_ip6,
	...
	else
	{
		...
		result = adns_submit_reverse(dns_state, (struct sockaddr *) &ipn.sins.sin, adns_r_ptr,
