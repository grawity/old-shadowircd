 ...
 *
 * $Id: patricia.c,v 1.1 2004/07/29 15:27:09 nenolod Exp $
...
patricia_node_t *
make_and_lookup_ip(patricia_tree_t * tree, struct irc_inaddr *ip, int bitlen)
{
	prefix_t *prefix;
	patricia_node_t *node;
	
	prefix = New_Prefix(DEF_FAM, &PIN_ADDR(ip), bitlen);
...
match_ip(patricia_tree_t * tree, struct irc_inaddr *ip)
{
	...
	if((prefix = New_Prefix(DEF_FAM, &PIN_ADDR(ip), len)) != NULL)
	{
