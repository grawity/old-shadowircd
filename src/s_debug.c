 ...
 *  USA
 *
 *  $Id: s_debug.c,v 1.1 2003/11/04 07:11:22 nenolod Exp $
 *
 */
...
count_memory (aClient * client_p, char *nick)
{
  ...
  for (target_p = client; target_p; target_p = target_p->next)
      ...
      if (MyConnect (target_p))
	{
	  ...
	  for (link = target_p->confs; link; link = link->next)
	    lcc++;
	}
      else
	rc++;
...
#ifdef FLUD
      for (link = target_p->fludees; link; link = link->next)
	fludlink++;
#endif
      if (target_p->serv)
	{
	...
	{
	  us++;
	  for (link = target_p->user->invited; link; link = link->next)
	    usi++;
	  for (link = target_p->user->channel; link; link = link->next)
	    usc++;
	  for (link = target_p->user->dccallow; link; link = link->next)
	    {
	  ...
	  for (link = target_p->user->silence; link; link = link->next)
	    uss++;
  ...
  for (channel_p = channel; channel_p; channel_p = channel_p->nextch)
	...
	chu++;
      for (link = channel_p->invites; link; link = link->next)
	chi++;
      for (bp = channel_p->banlist; bp; bp = bp->next)
	{
	  chb++;
	  chbm +=
	    (strlen (bp->who) + strlen (bp->banstr) + 2 + sizeof (aBan));
	}
    }
  ...
  sendto_one (client_p,
	      ":%s %d %s :whowas %d chan %d client/user %d misc %d dbuf %d hash %d res %d link %d flud %d",
	      me.name, RPL_STATSDEBUG, nick, totww, totch, totcl, totmisc, db,
	      tothash, rm, linkallocsz, fludallocsz);

  sendto_one (client_p, ":%s %d %s :TOTAL: %d sbrk(0)-etext: %d",
