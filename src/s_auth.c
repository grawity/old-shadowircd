 ...
 *  USA
 *
 *  $Id: s_auth.c,v 1.1 2003/11/04 07:11:28 nenolod Exp $
 *
 */
...
start_auth (aClient * client_p)
{
  ...
  if (client_p->authfd >= MAXCONNECTIONS)
    {
      sendto_snomask (SNOMASK_DEBUG, "Can't allocate fd for auth on %s",
  ...
  getsockname (client_p->fd, (struct SOCKADDR *) &localaddr, &locallen);
...
send_authports (aClient * client_p)
{
  ...
  if (getsockname (client_p->fd, (struct SOCKADDR *) &us, &ulen) ||
      getpeername (client_p->fd, (struct SOCKADDR *) &them, &tlen))
    {
#ifdef	USE_SYSLOG
