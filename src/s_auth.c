...
auth_dns_callback(void *vptr, adns_answer * reply)
{
	...
	else
	{
		   ...
		   && ConfigFileEntry.fallback_to_ip6_int == 1 && auth->ip6_int == 0)
		{
			...
			auth->ip6_int = 1;
			MyFree(reply);
			if(adns_getaddr(&client->localClient->ip, client->localClient->aftype, &auth->dns_query, 1))
			{
				ClearDNSPending(auth);
			}
			else {
...
start_auth(struct Client *client)
{
	...
	{
#ifdef IPV6
		/* Try ip6.int if configured to */
		if(client->localClient->aftype == AF_INET6
		   && ConfigFileEntry.fallback_to_ip6_int == 1 && auth->ip6_int == 0)
		{
			auth->ip6_int = 1;
			if(!adns_getaddr(&client->localClient->ip, client->localClient->aftype, &auth->dns_query, 1))
	...
	/* Not checking DNS or AUTH..let them free */
	if(!IsDoingAuth(auth) && !IsDNSPending(auth))
	{
		release_auth_client(auth);
		return;
...
/*
 * delete_auth_queries()
 * Cancel any pending DNS and AUTH requests
 */
void
