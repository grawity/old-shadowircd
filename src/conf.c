...
static int
subblock_handler(CONFIGENTRY * ce, struct ConfTable *table)
{
    struct ConfTable *ct = NULL;
...
conf_rehash(void)
{
    me_t           *hold_me = scalloc(sizeof(me_t), 1);	/* and
							 * keep_me_warm; */
    copy_me(&me, hold_me);
