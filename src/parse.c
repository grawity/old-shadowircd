 ...
 *  USA
 *
 *  $Id: parse.c,v 1.1 2003/11/04 07:11:33 nenolod Exp $
 *
 */
  ...
  if (IsServer (source_p) || IsMe (source_p))
    {
      ...
      sendto_snomask (SNOMASK_DEBUG, "Message for %s[%s] from %s",
...
static void
remove_unknown (aClient * client_p, char *sender, char *buffer)
{
  ...
  if (IsClient (client_p))
    {
      sendto_snomask (SNOMASK_DEBUG,
			  ...
			  buffer, get_client_name (client_p, FALSE), sender);
      return;
    }
  /*
  ...
  if (!strchr (sender, '.'))
  ...
  else
    {
      sendto_snomask (SNOMASK_DEBUG,
			  ...
			  get_client_name (client_p, HIDEME), sender);
      sendto_one (client_p, ":%s SQUIT %s :(Unknown prefix (%s) from %s)",
		  me.name, sender, buffer, get_client_name (client_p,
							    HIDEME));
    }
}
