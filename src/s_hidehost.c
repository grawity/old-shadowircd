/*
 *  UltimateIRCd - an Internet Relay Chat Daemon, src/s_hidehost.c
 *
 ...
 *  USA
 *
 *  $Id: s_hidehost.c,v 1.1 2003/11/04 07:11:31 nenolod Exp $
 *
 */
...
make_virthost (char *curr, char *host, char *new)
{
  ...
  if (parc2 == 4)
      ...
      if (strchr ("0123456789", parv2[3][len - 1]))
	{
	  ...
	  ircsprintf (mask, "%x.%x.%x.%xX",
		      hash[0], hash[1], hash[2], hash[3]);
	}
      else
	{
	  ircsprintf (mask, "%x.%x.%s.%s.%s",
...
void
make_opervirthost (char *new, char *host)
{
  strncpy (new, host, HOSTLEN);
  return;
}

/* tentative ipv6-(virt)host */

void
make_ipv6virthost (char *curr, char *host, char *new)
{
  char *parv[HOSTLEN + 1], s[HOSTLEN + 1];
  int parc = 0;

  strncpy (s, curr, HOSTLEN);
  parc = str2arr (parv, s, ".");

  if (parc == 4)		/* ipv4 ^^ .. lets encrypt it */
    {
      make_virthost (curr, host, new);
      return;
    }
  else				/* ipv6 ! */
    {
