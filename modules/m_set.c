 ...
 *  USA
 *
 *  $Id: m_set.c,v 1.1 2004/07/29 15:27:39 nenolod Exp $
 */
...
static void quote_operstring(struct Client *, char *);
static void quote_adminstring(struct Client *, char *);
...
/* SET OPERSTRING */
static void
quote_operstring(struct Client *source_p, char *operstring)
{
	
	if(EmptyString(operstring))
	{
	...
	{
		if(ConfigFileEntry.operstring_admin_only && !IsAdmin(source_p))
		{
			sendto_one(source_p, ":%s NOTICE %s :setting OPERSTRING requires admin privledges",
				me.name, source_p->name);
			return;
		}

		strlcpy(GlobalSetOptions.operstring, operstring, 
			sizeof(GlobalSetOptions.operstring));
		sendto_realops_flags(UMODE_ALL, L_ALL, 
				     "%s has changed OPERSTRING to '%s'", 
				     get_oper_name(source_p), operstring);
	}
}

/* SET ADMINSTRING */
static void
quote_adminstring(struct Client *source_p, char *adminstring)
{

	if(EmptyString(adminstring))
	{
	...
	else
	{
		if(ConfigFileEntry.operstring_admin_only && !IsAdmin(source_p))
		{
			sendto_one(source_p, ":%s NOTICE %s :setting ADMINSTRING requires admin privledges",
				me.name, source_p->name);
			return;
		}
		strlcpy(GlobalSetOptions.adminstring, adminstring, 
			sizeof(GlobalSetOptions.adminstring));
		sendto_realops_flags(UMODE_ALL, L_ALL, 
				     "%s has changed ADMINSTRING to '%s'", 
				     get_oper_name(source_p), adminstring);
	}
}
