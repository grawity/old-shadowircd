 ...
 *  USA
 *
 *  $Id: m_rehash.c,v 1.1 2004/07/29 15:27:39 nenolod Exp $
 */
...
struct Message rehash_msgtab = {
	...
	{m_unregistered, m_not_oper, m_ignore, m_ignore, mo_rehash}
};
...
mo_rehash(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(parc > 1)
	{
		...
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     "%s is clearing G-lines", source_p->name);
			clear_temps(&glines);
		}
		else if(irccmp(parv[1], "PGLINES") == 0)
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     "%s is clearing pending glines", source_p->name);
			clear_pending_glines();
		}
		else if(irccmp(parv[1], "TKLINES") == 0)
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     "%s is clearing temp klines", source_p->name);
		...
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     "%s is clearing temp dlines", source_p->name);
			clear_temps(&tdline_min);
			clear_temps(&tdline_hour);
			clear_temps(&tdline_day);
			clear_temps(&tdline_week);
		}
		else if(irccmp(parv[1], "REJECTCACHE") == 0)
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     "%s is clearing reject cache", source_p->name);
			flush_reject();
		}
		else
		{
			sendto_one(source_p, ":%s NOTICE %s :rehash one of: "
				   "GLINES PGLINES TKLINES TDLINES REJECTCACHE DNS MOTD OMOTD",
				   me.name, source_p->name);
			return;
		}

		ilog(L_NOTICE, "REHASH %s From %s", parv[1], get_client_name(source_p, HIDE_IP));
		return;
	}
	else
	{
