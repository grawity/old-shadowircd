 ...
 *  USA
 *
 *  $Id: m_kline.c,v 1.1.1.1 2004/09/07 04:50:39 nenolod Exp $
 */
	  ...
	  int parc, char **parv)
{
  ...
  /* Because G:Lines are a silly hack, we'll do the same thing
   * by clustering K:Lines to everything by default.
   */
		      ...
		      const char *lhost, int warn)
{
  ...
  if ((aconf =
       find_conf_by_address (lhost, piphost, CONF_KILL, t, luser, NULL)))
    {
      if (warn)
	{
...
make_cidr (char *dlhost, struct Client *target_p)
{
  ...
  strlcpy (cidr_form_host,
	   inetntoa ((const char *) &target_p->localClient->ip),
  ...
  if (remove_tdline_match (cidr))
    {
      ...
      ilog (L_NOTICE, "%s removed temporary D-Line for [%s]", source_p->name,
	    cidr);
      return;
    }
