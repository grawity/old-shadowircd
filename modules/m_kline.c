 ...
 *  USA
 *
 *  $Id: m_kline.c,v 1.1 2004/07/29 15:27:43 nenolod Exp $
 */
...
static void handle_remote_kline(struct Client *source_p, const char *duration, 
		char *kuser, char *khost, char *kreason);
...
mo_kline(struct Client *client_p, struct Client *source_p, int parc, char **parv)
{
	...
	if(target_server != NULL)
	{
		propagate_generic(source_p, "KLINE", target_server, CAP_KLN,
				"%ld %s %s :%s",
				(long) tkline_time, user, host, reason);
		...
		cluster_generic(source_p, "KLINE", CLUSTER_KLINE, CAP_KLN,
				"%ld %s %s :%s",
				(long) tkline_time, user, host, reason);
...
ms_kline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	handle_remote_kline(source_p, parv[2], parv[3], parv[4], parv[5]);
}
...
me_kline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	handle_remote_kline(source_p, parv[1], parv[2], parv[3], parv[4]);
}

static void
handle_remote_kline(struct Client *source_p, const char *duration, 
		char *kuser, char *khost, char *kreason)
{
	const char *current_date;
