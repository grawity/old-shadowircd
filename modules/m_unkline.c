 ...
 *  USA
 *
 *  $Id: m_unkline.c,v 1.1 2004/07/29 15:27:35 nenolod Exp $
 */
...
struct Message msgtabs[] = {
	{"UNKLINE", 0, 0, 2, 0, MFLG_SLOW, 0,
	 {m_unregistered, m_not_oper, ms_unkline, me_unkline, mo_unkline}},
...
static void handle_remote_unkline(struct Client *, const char *, const char *);
...
mo_unkline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(parc > 3 && (irccmp(parv[2], "ON") == 0))
	{
		...
		if(match(parv[3], me.name) == 0)
		 ...
		 cluster_generic(source_p, "UNKLINE", CLUSTER_UNKLINE, CAP_UNKLN,
				 "%s %s", user, host);
...
static void
handle_remote_unkline(struct Client *source_p, const char *kuser, const char *khost)
{
