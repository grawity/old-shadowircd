 ...
 *  USA
 *
 *  $Id: m_stats.c,v 1.1 2004/07/29 15:27:38 nenolod Exp $
 */
...
struct Message stats_msgtab = {
	...
	{m_unregistered, m_stats, ms_stats, m_ignore, mo_stats}
};
...
stats_ziplinks(struct Client *source_p)
{
	...
	DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		if(IsCapable(target_p, CAP_ZIP))
		{
			...
			sendto_one(source_p,
				   ":%s %d %s Z :ZipLinks stats for %s send[%.2f%% compression (%lu kB data/%lu kB wire)] recv[%.2f%% compression (%lu kB data/%lu kB wire)]",
