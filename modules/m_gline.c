...
struct Message gline_msgtab = {
	...
	{m_unregistered, m_not_oper, ms_gline, m_ignore, mo_gline}
};
...
mo_gline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(ConfigFileEntry.glines)
	{
		...
		if((ptr = strchr(host, '/')) != NULL)
		{
			...
			if(strchr(host, ':') == NULL)
			{
				if(bitlen < ConfigFileEntry.gline_min_cidr)
				{
					sendto_one(source_p, ":%s NOTICE %s :Cannot set G-Lines with cidr length < %d", 
						me.name, parv[0], ConfigFileEntry.gline_min_cidr);
					return;
				}
			}
			...
			{
				sendto_one(source_p, ":%s NOTICE %s :Cannot set G-Lines with cidr length < %d", 
					me.name, parv[0], ConfigFileEntry.gline_min_cidr6);
				return;
			}
		...
		sendto_server(NULL, NULL, CAP_GLN, NOCAPS,
...
ms_gline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	sendto_server(client_p, NULL, NOCAPS, CAP_GLN,
		      ":%s GLINE %s %s %s %s %s %s :%s",
		      acptr->user->server, acptr->name, acptr->username,
		      acptr->host, acptr->user->server, user, host, reason);
	...
	if(ConfigFileEntry.glines)
	{
			...
			if(strchr(host, ':') == NULL)
			{
				if(bitlen < ConfigFileEntry.gline_min_cidr)
				{
					sendto_realops_flags(UMODE_ALL, L_ALL, "%s!%s@%s on %s is requesting "
						...
						acptr->name, acptr->username,
						acptr->host, acptr->user->server,
						ConfigFileEntry.gline_min_cidr, user, host, reason);
					return;
				}
			}
			else if(bitlen < ConfigFileEntry.gline_min_cidr6)
		...
		log_gline_request(acptr->name, acptr->username, acptr->host,
		...
		/* If at least 3 opers agree this user should be G lined then do it */
		check_majority_gline(source_p, acptr->name, acptr->username,
				     acptr->host, acptr->user->server,
				     user, host, reason);
	}
}
		  ...
		  const char *oper_server, const char *user, const char *host, const char *reason)
{
	...
	snprintf(filenamebuf, sizeof(filenamebuf), 
		 "%s.%s", ConfigFileEntry.glinefile, small_file_date((time_t) 0));
