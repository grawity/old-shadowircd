 ...
 *  USA
 *
 *  $Id: m_nick.c,v 1.5 2003/12/13 03:22:26 nenolod Exp $
 */
		  ...
		  char *parv[], time_t newts, char *nick)
{
  if (IsServer (source_p))
      ...
      else
	{
	  ...
	  ts_warn ("Remote nick %s (%s) introduced without a TS", nick,
		   parv[0]);
	}
 ...
 */
static int
client_from_server (struct Client *client_p, struct Client *source_p,
		    int parc, char *parv[], time_t newts, char *nick)
{
  char *m;
  unsigned int flag;
  const char *id = ID_ID;
