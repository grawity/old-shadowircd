 ...
 *  USA
 *
 *  $Id: m_kill.c,v 1.1 2004/07/29 18:29:19 nenolod Exp $
 */
...
mo_kill(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	char *user;
	static char noreason[] = "<No reason given>";
	const char *reason;
	...
	{
		sendto_one(source_p, ":%s NOTICE %s :You need local_kill = yes;", me.name, parv[0]);
		return;
	}
	...
	*buf = '\0';
	...
	if(IsOper(source_p))	/* send it normally */
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				      ...
				      source_p->host, source_p->username, source_p->name,
				      reason);
	}
	else
	{
	...
	DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		if(MyOper(source_p))
		{
			sendto_one(client_p, ":%s KILL %s :%s!%s!%s (%s)",
				   ...
				   source_p->host, source_p->username,
