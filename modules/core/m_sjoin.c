 ...
 *  USA
 *
 *  $Id: m_sjoin.c,v 1.1 2004/07/29 18:29:19 nenolod Exp $
 */
...
static void remove_a_mode(struct Channel *chptr, struct Client *source_p, int, char flag);
...
ms_sjoin(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	*parabuf = '\0';
	...
	/* bursting an SJOIN with no users? */
	s = parv[4+args];
	while(*s == ' ')
		s++;
	...
	*mbuf = '\0';
	if(pargs)
	{
		sendto_channel_local(ALL_MEMBERS, chptr,
				     ":%s MODE %s %s %s %s %s %s",
				     server_name,
				     chptr->chname, modebuf, para[0], para[1], para[2], para[3]);
	}

	if(!people)
	{
		/* we created the channel, not joining anyone to it, so nuke
		 * it. --fl
		 */
		if(isnew)
	...
	sendto_server(client_p->from, NULL, NOCAPS, NOCAPS,
		      "%s%s", buf, sjbuf);
}
