 ...
 *  USA
 *
 *  $Id: m_kick.c,v 1.1 2004/07/29 18:29:19 nenolod Exp $
 */
...
struct Message kick_msgtab = {
	...
	{m_unregistered, m_kick, ms_kick, m_ignore, m_kick}
};
...
m_kick(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	*buf = '\0';
	...
	msptr2 = find_channel_membership(chptr, who);
	if(msptr2 != NULL)
	{
		if (is_chanop(msptr2) && is_halfop(msptr))
		{
			sendto_one(source_p, form_str(ERR_CHANOPRIVSNEEDED),
		...
		remove_user_from_channel(msptr2);
	}
	else
