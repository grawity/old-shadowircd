 ...
 *  USA
 *
 *  $Id: m_xline.c,v 1.1.1.1 2004/09/07 04:50:39 nenolod Exp $
 */
	  ...
	  int parc, char *parv[])
{
  ...
  if ((conf = find_matching_name_conf (XLINE_TYPE, parv[1],
				       NULL, NULL, 0)) != NULL)
    {
      ...
      sendto_one (source_p,
		  ":%s NOTICE %s :[%s] already X-Lined by [%s] - %s", me.name,
		  source_p->name, parv[1], conf->name, match_item->reason);
      return;
    }
	  ...
	  int parc, char *parv[])
{
  ...
  if ((find_matching_name_conf (XLINE_TYPE, parv[2], NULL, NULL, 0)) != NULL)
    return;
  ...
  if ((conf = find_matching_name_conf (XLINE_TYPE, parv[2],
				       NULL, NULL, 0)) != NULL)
    {
      ...
      sendto_one (source_p,
		  ...
		  ID_or_name (&me, source_p->from), ID_or_name (source_p,
								source_p->
								from),
